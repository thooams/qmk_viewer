...
fn test_keymap_parsing(keyboard_name: &str) -> CompatibilityResult {
    ...
    match fs::read_to_string(&file_path) {
        ...
        Err(e) => {
            let parse_time = start_time.elapsed().as_millis() as u64;
            let clean_name = keyboard_name.replace("_keymap.c", "");
            CompatibilityResult::failure(clean_name, format!("File read error: {}", e), parse_time)
        }
    }
}
...
fn test_all_qmk_keyboards() {
    ...
    fs::write("tests/compatibility_report.md", &report)
    ...
    assert!(
        successful_count > 0,
        "No keyboards were successfully parsed!"
    );
    ...
    if success_rate < 50.0 {
        println!(
            "⚠️  Warning: Success rate is below 50% ({:.1}%)",
            success_rate
        );
    }
}
...
fn test_sample_keyboards() {
    ...
    let sample_keyboards = vec!["planck_keymap.c", "test_keymap.c"];
