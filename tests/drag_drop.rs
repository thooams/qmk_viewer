use qmk_viewer::config_persistence::{clear_saved_keymap, get_saved_keymap_path, save_keymap_file};
use std::fs;
...
fn test_config_persistence() {
    ...
    // Verify it's cleared
    let cleared_path =
        get_saved_keymap_path().expect("Failed to get saved keymap path after clear");
